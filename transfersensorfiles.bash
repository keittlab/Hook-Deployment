...
cat $SENSORFILES_DIR/../logs/sensor-xfer-temp.log-$TIMESTAMP | while read line; do
        if [[ `(/usr/bin/python3 $SOUNDFILES_DIR/../stengl-minio-tests/stengl-minio-md5check-cleanoutpu>              echo "File $SENSORFILES_DIR/$line already sync'd"
        else
               /usr/bin/python3 $SOUNDFILES_DIR/../stengl-minio-tests/sendtocorral-minio.py $SENSORFILE>        fi;
done
